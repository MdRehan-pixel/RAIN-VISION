...
export default function LeafletMap({ location, radiusKm, spatial, cities, onSelect, large = false }: { location: LocationState; radiusKm: number; spatial: SpatialPoint[]; cities: CityWeather[]; onSelect: (latitude: number, longitude: number) => void; large?: boolean }) {
  ...
  return <div data-testid="risk-map" className={`relative overflow-hidden rounded-2xl border border-slate-200 bg-slate-100 shadow-sm ${large ? "h-[calc(100vh-220px)] min-h-[520px]" : "h-[390px]"}`}>
    ...
    <div data-testid="map-coordinates-hud" className="pointer-events-none absolute bottom-3 left-3 z-[400] rounded-xl border border-white/80 bg-white/95 px-3 py-2 shadow-lg backdrop-blur"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500">Selected coordinates</p><p className="mt-1 font-mono text-xs text-slate-900">{formatNumber(location.latitude, 4)}° N · {formatNumber(location.longitude, 4)}° E</p></div>
