...
import { ChevronRight, History, MapPinned, Pause, Play, RotateCcw } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import LeafletMap from "@/components/LeafletMap";
import { apiGet } from "@/lib/api";
import { FACTOR_LABELS, buildReplaySteps, contributionLabel, formatReplayDateTime, summariseReplay, type ReplayStep } from "@/lib/historical";
import { RISK_WEIGHTS, riskColor } from "@/lib/risk";
import type { HistoricalIncident, HistoricalStatus, IncidentListResponse, LocationState, RiskCategory, RiskFactors } from "@/lib/types";

type ReplayState = "IDLE" | "PLAYING" | "PAUSED" | "COMPLETE";
...
function StatusPill({ status, label }: { status: HistoricalStatus; label?: string }) {
...
function Stat({ label, value, sub, color, testId }: { label: string; value: string; sub?: string; color?: string; testId: string }) {
  return <div className="rounded-xl border border-slate-200 bg-white p-4"><p className="font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">{label}</p><p data-testid={testId} className="mt-1 font-mono text-2xl font-bold tracking-tight" style={{ color: color ?? "#0f172a" }}>{value}</p>{sub && <p className="mt-1 text-xs text-slate-500">{sub}</p>}</div>;
}
...
export default function HistoricalReplay() {
  ...
  const reset = () => { stop(); setStepIndex(-1); setState("IDLE"); };
  const play = () => {
    ...
    timer.current = window.setInterval(() => {
      setStepIndex((index) => {
        if (index >= steps.length - 1) { stop(); setState("COMPLETE"); return index; }
        return index + 1;
      });
    }, STEP_INTERVAL_MS);
  };
  const pause = () => { stop(); setState("PAUSED"); };
  const next = () => { stop(); setStepIndex((index) => { const value = Math.min(index + 1, steps.length - 1); setState(value >= steps.length - 1 ? "COMPLETE" : "PAUSED"); return value; }); };
  useEffect(() => () => stop(), []);
  useEffect(() => { reset(); }, [activeId]); // eslint-disable-line react-hooks/exhaustive-deps -- reset replay when the incident changes

  const current: ReplayStep | undefined = stepIndex >= 0 ? steps[stepIndex] : undefined;
  const shown = stepIndex >= 0 ? steps.slice(0, stepIndex + 1) : [];
  const summary = useMemo(() => (state === "COMPLETE" ? summariseReplay(steps) : null), [state, steps]);
  const chartData = shown.map((step) => ({ label: step.label, rainfall: Number(step.windowTotal.toFixed(1)), peak: Number(step.intensity.toFixed(1)), risk: step.risk.score, inundation: step.inundation.score }));
  const mapLocation: LocationState | null = dataset ? { name: `${dataset.incident.location.city} · historical incident`, state: dataset.incident.location.state, country: dataset.incident.location.country, latitude: dataset.incident.location.latitude, longitude: dataset.incident.location.longitude, source: "SEARCH" } : null;
  const categoryColor = (category: RiskCategory) => riskColor(category);
  ...
  return <div data-testid="historical-replay-page">
    ...
    <div className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
      <Card data-testid="incident-card" className="border-slate-200 shadow-sm">
        ...
        <CardContent>
          <div className="mb-4 flex flex-wrap gap-2" data-testid="incident-selector">{incidents.map((item) => <button key={item.id} data-testid={`incident-option-${item.id}`} onClick={() => setSelectedId(item.id)} aria-pressed={item.id === activeId} className={`rounded-lg border px-3 py-2 text-left text-xs font-semibold transition-colors ${item.id === activeId ? "border-slate-900 bg-slate-900 text-white" : "border-slate-200 bg-white text-slate-700 hover:bg-slate-50"}`}>{item.name}{item.primaryDemo && <span className="ml-2 font-mono text-[9px] uppercase tracking-wider opacity-70">Primary demo</span>}</button>)}{incidentsQuery.isLoading && <span className="text-xs text-slate-500">Loading incident list…</span>}{!incidentsQuery.isLoading && !incidents.length && <span className="text-xs text-slate-500">No historical dataset available.</span>}</div>
          {dataset && <>
            ...
            <div className="flex items-center justify-between py-2.5 text-sm"><span className="font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-slate-500">Status</span><StatusPill status="HISTORICAL" label={state === "IDLE" ? "Ready for replay" : state === "COMPLETE" ? "Replay complete" : `Replay ${state.toLowerCase()}`} /></div>
            <div className="mt-4 flex flex-wrap gap-2">
              <Button data-testid="button-run-historical-replay" onClick={() => { reset(); window.setTimeout(play, 0); }} disabled={!steps.length}><Play size={14} />Run historical replay</Button>
              <Button data-testid="button-reset-replay" variant="outline" onClick={reset} disabled={state === "IDLE"}><RotateCcw size={14} />Reset replay</Button>
            </div>
          </>}
        </CardContent>
      </Card>
      <div className="relative">{mapLocation && <LeafletMap location={mapLocation} radiusKm={10} spatial={[]} cities={[]} onSelect={() => undefined} hudLabel="Historical incident location" />}<div className="pointer-events-none absolute left-3 top-3 z-[400] rounded-lg border border-white/80 bg-white/95 px-3 py-1.5 font-mono text-[10px] font-bold uppercase tracking-[0.14em] text-slate-700 shadow"><MapPinned size={11} className="mr-1 inline" />Historical incident location</div></div>
    </div>

    {dataset && <>
      <Card data-testid="historical-inputs-card" className="mt-5 border-slate-200 shadow-sm">
      ...
      <Card data-testid="replay-card" className="mt-5 border-slate-200 shadow-sm">
        <CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle className="text-lg">Replay timeline</CardTitle><CardDescription>{steps.length} timesteps · {dataset.replay?.note ?? "Daily steps"}</CardDescription></div><div className="flex flex-wrap gap-2">{state === "PLAYING" ? <Button data-testid="button-replay-pause" size="sm" variant="outline" onClick={pause}><Pause size={14} />Pause</Button> : <Button data-testid="button-replay-play" size="sm" variant="outline" onClick={play} disabled={!steps.length || state === "COMPLETE"}><Play size={14} />Play replay</Button>}<Button data-testid="button-replay-next" size="sm" variant="outline" onClick={next} disabled={!steps.length || state === "COMPLETE"}><ChevronRight size={14} />Next step</Button><Button data-testid="button-replay-reset" size="sm" variant="ghost" onClick={reset} disabled={state === "IDLE"}><RotateCcw size={14} />Reset</Button></div></div></CardHeader>
        <CardContent>
          <div className="grid gap-3 sm:grid-cols-3">
            <Stat label="Current replay time" value={current ? current.label : "—"} sub={current ? `${current.windowHours} h window · step ${stepIndex + 1} of ${steps.length}` : "Run the replay to start"} testId="replay-current-time" />
            <Stat label="Current risk score" value={current ? `${current.risk.score} / 100` : "—"} sub={current ? `Inputs available: ${Math.round(current.risk.availableWeight * 100)}% of nominal weight` : undefined} color={current ? categoryColor(current.risk.category) : undefined} testId="replay-current-score" />
            <Stat label="Current risk level" value={current ? current.risk.category : "—"} sub={current ? `Alert engine: ${current.alertStatus}` : undefined} color={current ? categoryColor(current.risk.category) : undefined} testId="replay-current-level" />
          </div>
          <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-100"><div data-testid="replay-progress" className="h-full rounded-full bg-slate-900 transition-[width] duration-300" style={{ width: `${steps.length ? ((stepIndex + 1) / steps.length) * 100 : 0}%` }} /></div>
          <div className="mt-5 grid gap-5 xl:grid-cols-2">
            <div data-testid="chart-rainfall-vs-time"><p className="mb-2 text-sm font-semibold text-slate-800">Rainfall vs time <span className="font-mono text-[10px] font-normal text-slate-500">· mm per step (ERA5)</span></p><div className="h-[240px]">{chartData.length ? <ResponsiveContainer width="100%" height="100%"><BarChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} /><XAxis dataKey="label" tick={{ fontSize: 9 }} interval="preserveStartEnd" tickLine={false} axisLine={false} /><YAxis tick={{ fontSize: 10 }} tickLine={false} axisLine={false} width={30} /><Tooltip content={<ReplayTooltip />} /><Bar dataKey="rainfall" name="Rainfall mm" fill="#0284c7" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer> : <div className="grid h-full place-items-center rounded-xl border border-dashed border-slate-200 text-xs text-slate-500">Rainfall series appears as the replay runs.</div>}</div></div>
            <div data-testid="chart-risk-vs-time"><p className="mb-2 text-sm font-semibold text-slate-800">Risk score & inundation vs time <span className="font-mono text-[10px] font-normal text-slate-500">· threshold zones</span></p><div className="h-[240px]">{chartData.length ? <ResponsiveContainer width="100%" height="100%"><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" vertical={false} /><ReferenceArea y1={0} y2={30} fill="#059669" fillOpacity={0.06} /><ReferenceArea y1={30} y2={50} fill="#d97706" fillOpacity={0.07} /><ReferenceArea y1={50} y2={70} fill="#ea580c" fillOpacity={0.08} /><ReferenceArea y1={70} y2={100} fill="#dc2626" fillOpacity={0.08} /><ReferenceLine y={30} stroke="#d97706" strokeDasharray="4 4" label={{ value: "MODERATE", fontSize: 9, position: "insideTopLeft", fill: "#92400e" }} /><ReferenceLine y={50} stroke="#ea580c" strokeDasharray="4 4" label={{ value: "HIGH", fontSize: 9, position: "insideTopLeft", fill: "#9a3412" }} /><ReferenceLine y={70} stroke="#dc2626" strokeDasharray="4 4" label={{ value: "EXTREME", fontSize: 9, position: "insideTopLeft", fill: "#991b1b" }} /><XAxis dataKey="label" tick={{ fontSize: 9 }} interval="preserveStartEnd" tickLine={false} axisLine={false} /><YAxis domain={[0, 100]} tick={{ fontSize: 10 }} tickLine={false} axisLine={false} width={30} /><Tooltip content={<ReplayTooltip />} /><Line type="monotone" dataKey="risk" name="Risk score" stroke="#0f172a" strokeWidth={2.5} dot={false} isAnimationActive={false} /><Line type="monotone" dataKey="inundation" name="Inundation risk" stroke="#d97706" strokeWidth={2} dot={false} isAnimationActive={false} /></LineChart></ResponsiveContainer> : <div className="grid h-full place-items-center rounded-xl border border-dashed border-slate-200 text-xs text-slate-500">Risk evolution appears as the replay runs.</div>}</div></div>
          </div>
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        <Card data-testid="historical-incident-card" className="border-slate-200 shadow-sm">
        ...
        <Card data-testid="rain-vision-output-card" className={`border-slate-200 shadow-sm ${current && (current.risk.category === "HIGH" || current.risk.category === "EXTREME") ? "border-l-4 border-l-rose-500" : ""}`}>
          ...
          <CardContent>
            {current ? <>
              <div className="grid grid-cols-2 gap-3">
                <Stat label="Risk score" value={`${current.risk.score} / 100`} color={categoryColor(current.risk.category)} testId="output-risk-score" />
                <Stat label="Heavy rainfall risk" value={current.risk.category} color={categoryColor(current.risk.category)} testId="output-risk-category" />
                <Stat label="Prototype inundation risk" value={current.inundation.category} sub={`${current.inundation.score} / 100 · ${current.inundation.trend}`} color={categoryColor(current.inundation.category)} testId="output-inundation" />
                <Stat label="Alert status" value={current.alertStatus} color={current.alertStatus === "NO ALERT" ? "#0f172a" : categoryColor(current.risk.category)} testId="output-alert-status" />
              </div>
      ...
      {summary && <Card data-testid="backtest-comparison-card" className="mt-5 border-slate-200 shadow-sm">
        ...
        <CardContent className="grid gap-5 lg:grid-cols-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Stat label="Highest calculated risk" value={`${summary.peakStep.risk.score} / 100`} sub={summary.peakStep.label} color={categoryColor(summary.peakStep.risk.category)} testId="summary-peak-risk" />
            <Stat label="Peak risk level" value={summary.peakStep.risk.category} sub={`${summary.stepsAbove.HIGH + summary.stepsAbove.EXTREME} of ${steps.length} steps ≥ HIGH`} color={categoryColor(summary.peakStep.risk.category)} testId="summary-peak-level" />
            <Stat label="Peak inundation risk" value={summary.peakInundationStep.inundation.category} sub={`${summary.peakInundationStep.inundation.score} / 100 · ${summary.peakInundationStep.label}`} color={categoryColor(summary.peakInundationStep.inundation.category)} testId="summary-peak-inundation" />
      ...
      <Card data-testid="how-this-test-works-card" className="mt-5 border-slate-200 shadow-sm">
